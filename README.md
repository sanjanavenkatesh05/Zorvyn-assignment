...
*   **/finance_backend**: The FastAPI backend application and Docker configuration.
*   **/postman**: The pre-configured Postman Collection for automated API testing.
...
1. You must have a MongoDB instance running locally (port `27017`) or configure the `MONGO_URI` inside `finance_backend/.env` to point to a MongoDB Atlas cluster.
2. Enter the backend directory:
   ```bash
   cd finance_backend
   ```
